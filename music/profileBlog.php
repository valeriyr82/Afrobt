...
$CFG['lang']['include_files'][] = 'languages/%s/music/profileBlog.php';
$CFG['lang']['include_files'][] = 'languages/%s/music/profileTabText.php';
...
if(class_exists('MusicHandler'))
	{
		class ProfileMusicHandler extends MusicHandler{}
	}
class BloggerHandler extends ProfileMusicHandler
	{
...
if(isset($_SERVER['HTTP_REFERER']) AND
		strpos($_SERVER['HTTP_REFERER'],'viewmusic'))
		{
...
$BloggerHandler->setFormField('music_id', '');
$BloggerHandler->view_music_url = '';
...
if($BloggerHandler->getFormField('music_id') != '')
	$BloggerHandler->view_music_url = getUrl('viewmusic', '?music_id='.$BloggerHandler->getFormField('music_id'),
										$BloggerHandler->getFormField('music_id').'/', '', 'music');
...
if ($BloggerHandler->isShowPageBlock('block_blogger_list'))
	{
		...
		$BloggerHandler->block_blogger_list['confirm_form_hidden_arr'] = array('backkey', 'music_id');
...
<script type="text/javascript" language="javascript">
	...
	var openAddNewBlogWindow = function(){
		...
		url = '<?php echo $CFG['site']['music_url'].'profileBlog.php';?>';
		pars = add_pars+'act=open_blogger_list&music_id=<?php echo $BloggerHandler->getFormField('music_id');?>&backkey=<?php echo $BloggerHandler->getFormField('backkey');?>';
		$Jq.ajax({
					type	: 'POST',
					url		: url,
					data	: pars,
					success	: eval('openAddNewBlogWindowResponse')
				});
		return false;
	}
	...
	function showBlogDetailForm(){
		var url = '<?php echo $CFG['site']['music_url'].'profileBlog.php';?>';
