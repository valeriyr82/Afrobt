...
class transactionDetails extends MusicHandler
			...
			while($row = $this->fetchResultRecord())
				{
					...
					$showAlbumlists_arr['row'][$inc]['total_price'] = $row['total_price'];
			...
			while($row = $this->fetchResultRecord())
				{
					...
					$showMusiclists_arr['row'][$inc]['total_price'] = $row['total_price'];
					$showMusiclists_arr['row'][$inc]['user_id'] = $row['user_id'];
					$showMusiclists_arr['row'][$inc]['music_album_id'] = $row['music_album_id'];
					$showMusiclists_arr['row'][$inc]['music_id'] = $row['music_id'];
					$showMusiclists_arr['row'][$inc]['total_purchases'] = $row['total_purchases'];
					$showMusiclists_arr['row'][$inc]['word_wrap_music_title'] = highlightWords($row['music_title'],$this->fields_arr['albumlist_title']);
		...
		*/
		public function setTableAndColumns()
		{
			...
			switch ($this->fields_arr['transaction_type'])
			{
				...
				$this->setReturnColumns(array('count(mp.album_id) as total_purchases','sum(mp.price) as total_price','al.music_album_id', 'al.album_title', 'al.date_added', 'al.total_album_views as total_views', 'al.album_for_sale','al.user_id','al.album_price'));
				$this->sql_condition = 'mp.owner_id = '.$this->CFG['user']['user_id'].$this->albumlistAdvancedFilters().' GROUP BY al.music_album_id';
				...
				$this->setReturnColumns(array('count(mp.music_id) as total_purchases','sum(mp.price) as total_price','al.music_album_id','m.music_id','m.music_title', 'm.music_price', 'm.date_added', 'total_views', 'for_sale','m.user_id'));
				$this->sql_condition = 'mp.owner_id = '.$this->CFG['user']['user_id'].' AND price > 0 '.$this->albumlistAdvancedFilters().' GROUP BY mp.music_id';
		...
		*/
		...
		public function displayAlbumDetailsList($album_id='')
		{
			...
			if($row = $rs->FetchRow())
			{
				...
				$displayDetails_arr['music'] = false;
				$displayDetails_arr['title'] = $row['album_title'];
				$displayDetails_arr['music_album_id'] = $album_id;
		...
		public function displayMusicDetailsList($music_id='')
		{
			...
			if($row = $rs->FetchRow())
			{
				...
				$displayDetails_arr['music_id'] = $music_id;
	...
	public function populatePaymentDetails()
	{
		...
		if($row = $rs->FetchRow())
		{
			...
			$paymentList_arr['total_sale_amount'] = $paymentList_arr['total_revenue']+$paymentList_arr['commission_amount'];
		}
		$smartyObj->assign('paymentList_arr', $paymentList_arr);

	}

	public function totalAlbumSaleAmount($album_id)
	{
		$price = 0;
		$sql = 'SELECT sum(price) as price FROM '.
				$this->CFG['db']['tbl']['music_album_purchase_user_details'].
				' WHERE album_id='.$this->dbObj->Param('album_id');
		$stmt = $this->dbObj->Prepare($sql);
		$rs = $this->dbObj->Execute($stmt, array($album_id));
	...
	public function totalMusicSaleAmount($music_id)
	{
		$price = 0;
		$sql = 'SELECT sum(price) as price FROM '.
				$this->CFG['db']['tbl']['music_purchase_user_details'].
...
$album_url = getUrl('transactionlist','?transaction_type=album','?transaction_type=album','members','music');
$music_url = getUrl('transactionlist','?transaction_type=music','?transaction_type=music','members','music');
