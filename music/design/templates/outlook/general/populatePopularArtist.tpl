...
{include file="box.tpl" opt="sidebar_top"}
    <div class="clsAudioIndex clsIndexPopularArtist">
        <span class="clsWhatgoingHeading">{$LANG.sidebar_popular_artist_label}</span>
		...
		<div class="clsOverflow clsPopularArtistInnerpad">
           {foreach item=artistValue from=$populatePopularArtist_arr.row }
            ...
            <p class="clsViewMore"><a href="{$moreartist_url}" title="{$LANG.sidebar_viewall_label}" class="viewmore">{$LANG.sidebar_viewall_label}</a></p>
         {else}
