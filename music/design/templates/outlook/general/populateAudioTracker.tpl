...
{include file="box.tpl" opt="sidebar_top"}
    <div class="clsAudioIndex clsAudioTrackerBlock">
    	<span class="clsWhatgoingHeading">{$LANG.sidebar_audio_tracker_label}</span>
        {if $populateAudioTracker_arr.record_count}
           {foreach item=trackerValue from=$populateAudioTracker_arr.row }
            ...
            {/foreach}
            <p class="clsViewMore"><a href="{$audiotracker_url}" alt="{$LANG.sidebar_music_more_label}" title="{$LANG.sidebar_music_more_label}" class="viewmore">{$LANG.sidebar_music_more_label}</a></p>
         {else}
