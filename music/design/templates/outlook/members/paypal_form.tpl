<form name="form_register" id="selFormRegister" action="{if $CFG.payment.paypal.test_mode}{$CFG.payment.paypal.test_url}{else}{$CFG.payment.paypal.url}{/if}" target="{if $myobj->getFormField('status')} _self {else}mycartwindow{/if}" onSubmit="{if !$myobj->getFormField('status')}window.close(){/if}" method="post">
	...
	<div class="clsSubmitButton-l"><span class="clsSubmitButton-r"><input name="pay_paypal" id="pay_paypal" type="submit" value="{$payment_details.paypal.submit_value}"></span></div>
	</div>
</form>