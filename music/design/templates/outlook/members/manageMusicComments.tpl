        ...
        <div class="clsHeadingRight">
				<form name="commentStatusForm" id="commentStatusForm" method="post" action="{$myobj->getCurrentUrl()}" autocomplete="off">
							<select name="comment_status" id="comment_status" tabindex="{smartyTabIndex}" onchange="return changeCommentStatus(this.value)">
								<option value="" {if $myobj->getFormField('comment_status') == ''}Selected{/if}>{$LANG.managemusiccomments_selectbox_all}</option>
								<option value="{$LANG.common_yes_option}" {if $myobj->getFormField('comment_status') == $LANG.common_yes_option}Selected{/if}>{$LANG.managemusiccomments_activate}</option>
								<option value="{$LANG.common_no_option}" {if $myobj->getFormField('comment_status') == $LANG.common_no_option}Selected{/if}>{$LANG.managemusiccomments_inactivate}</option>																		
							</select>
				</form>		          	
        </div>
    </div>
                    ...
                    <div class="clsDataTable">
						...
						<table summary="{$myobj->form_manage_comments.comments_tbl_summary}" class="clsMyMusicAlbumTbl">
						  <tr>
							...
							<th class="clsWidth90">{$LANG.managemusiccomments_by}</th>
							<th class="clsWidth75">{$LANG.managemusiccomments_date}</th>
							<th class="clsWidth55">{$LANG.managemusiccomments_status}</th>
							<th>{$LANG.managemusiccomments_option}</th>
						  </tr>
								...
								<tr class="{$value.tr_class}">	
								  ...
								  <td>
                                  		{if $value.comment_status == 'Yes'}
                                	  		{$LANG.managemusiccomments_activate}
	                                    {elseif $value.comment_status == 'No'}
	                            	  		{$LANG.managemusiccomments_inactivate}
                                        {/if}
                                  </td>
								  <td><span>
...
<span class="clsSubmitButton-r"><input type="button" class="clsSubmitButton" name="delete_submit" id="delete_submit" tabindex="{smartyTabIndex}" value="{$LANG.managemusiccomments_delete}" onClick="if(getMultiCheckBoxValue('commendsForm', 'check_all', '{$LANG.common_check_atleast_one}', 'dAltMulti', -100, -500))Confirmation('selMsgConfirm', 'confirm_form', Array('comment_id','act', 'selConfirmMsg'), Array(multiCheckValue, 'delete', '{$LANG.managemusiccomments_delete_confirmation}'), Array('value','value', 'innerHTML'), 0, 0);" /></span></p>
								{*if $myobj->getFormField('comment_status') != '' && $myobj->getFormField('comment_status') != $LANG.common_yes_option*}
									<p class="clsSubmitButton-l">
<span class="clsSubmitButton-r"><input type="button" class="clsSubmitButton" name="activate" id="activate" tabindex="{smartyTabIndex}" value="{$LANG.managemusiccomments_activate_button_label}" onClick="if(getMultiCheckBoxValue('commendsForm', 'check_all', '{$LANG.common_check_atleast_one}', 'dAltMulti', -100, -500))Confirmation('selMsgConfirm', 'confirm_form', Array('comment_id','act', 'selConfirmMsg'), Array(multiCheckValue, 'activate', '{$LANG.managemusiccomments_activate_confirmation}'), Array('value','value', 'innerHTML'), 200, 100);" /></span></p>
								{*/if*}</div>
						  </td>
						    <td style="padding:0">
