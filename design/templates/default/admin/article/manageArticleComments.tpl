            	...
            	<form name="form_comments_list" id="form_comments_list" method="post" action="{$myobj->getCurrentUrl()}" autocomplete="off">
                	<table summary="{$LANG.tbl_summary}" class="clsFormTableSection">
                    	<tr>
                            ...
                            <th>{$LANG.articlecomment_commented_user}</th>
                            <th>{$LANG.articlecomment_comment_description}</th>
                            <th>{$LANG.articlecomment_date_added}</th>
                            <th>
                        ...
                        {foreach key=pcKey item=pcValue from=$populateComments_arr}
                            <tr class="{$myobj->getCSSRowClass()}">
                                <td><input type="checkbox" class="clsCheckRadio" name="cid[]" value="{$pcValue.record.article_comment_id}" tabindex="{smartyTabIndex}" /></td>
                                <td><p class="clsImage"><a href="{$pcValue.commentUserProfileUrl}"><img src="{$pcValue.profileIcon.t_url}" alt="{$pcValue.UserDetails}" title="{$pcValue.UserDetails}" {$myobj->DISP_IMAGE(#image_thumb_width#, #image_thumb_height#, $pcValue.profileIcon.t_width, $pcValue.profileIcon.t_height)} /></a></p></td>
                                <td><textarea rows="4" cols="50" name="commentText_{$pcValue.record.article_comment_id}">{$pcValue.record.comment}</textarea></td>
                                <td>{$pcValue.record.date_added}</td>
                                <td><input type="submit" class="clsSubmitButton" name="updateSubmit" id="updateSubmit" value="{$LANG.update}" tabindex="{smartyTabIndex}" onClick="changeSubmitText({$pcValue.record.article_comment_id})" /></td>
                            </tr>
                        {/foreach}
                        <tr>
                            <td colspan="4">
