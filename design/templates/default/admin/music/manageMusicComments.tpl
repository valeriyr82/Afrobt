                ...
                <form name="form_comments_list" id="form_comments_list" method="post" action="{$myobj->getCurrentUrl()}" autocomplete="off">
                    ...
                    <table summary="{$LANG.tbl_summary}" class="clsFormTableSection">
                        <tr>
                            ...
                            <th>{$LANG.managemusiccomments_status_label}</th>
                            <th>
...
&nbsp;                            </th>
                        ...
                        {literal}
							<script language="javascript">
                                ...
                                comment_id[{/literal}{$count}{literal}] = {/literal}'{$pcValue.record.music_comment_id}'{literal};
                            </script>
                         {/literal}    
                            <tr class="{$myobj->getCSSRowClass()}">
                                <td class="clsSelectAllItems"><input type="checkbox" class="clsCheckRadio" name="cid[]" value="{$pcValue.record.music_comment_id}" tabindex="{smartyTabIndex}" onClick="disableHeading('form_comments_list');"/></td>
                                ...
                                <td><textarea rows="4" cols="50" id="commentText_{$pcValue.record.music_comment_id}" name="commentText_{$pcValue.record.music_comment_id}"  wrap="on">{$pcValue.record.comment}</textarea></td>
                                <td>{$pcValue.record.date_added}</td>
                                <td>
                                	{if $pcValue.record.comment_status == 'Yes'}
                                		{$LANG.managemusiccomments_show_label}
                                    {elseif $pcValue.record.comment_status == 'No'}
                                    	{$LANG.managemusiccomments_hide_label}
                                    {/if}   
                                </td>
                        ...
                        {/foreach}                        
                        <tr>
							 <td colspan="6"><input type="submit" class="clsSubmitButton" name="updateSubmit" id="updateSubmit" value="{$LANG.update}" tabindex="{smartyTabIndex}" onClick="changeSubmitText({$pcValue.record.music_comment_id})" />
