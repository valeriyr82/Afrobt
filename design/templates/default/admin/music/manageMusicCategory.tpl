	...
	<div id="selCreateCategory">
		...
		<table class="clsNoBorder" summary="{$LANG.managemusiccategory_create_tbl_summary}">
            ...
            <tr>
				...
				<td class="{$myobj->getCSSFormFieldCellClass('category_image')}">
			       {if $myobj->getFormField('category_id') && $myobj->getFormField(music_category_ext)!=''}
						<img src="{$myobj->category_image}" alt="{$myobj->getFormField('category_image')}" />
					{/if}
					{if $myobj->getFormField(music_category_ext)=='' && $myobj->getFormField('category_id')}
        ...
        {if $myobj->chkAllowedModule(array('content_filter'))}
			<tr id="selDateLocationRow" class="clsAllowOptions">
         		<td class="{$myobj->getCSSFormLabelCellClass('allow_post')}"><label for="allow_post1">{$LANG.managemusiccategory_allow_post}</label></td>
			...
			<tr id="selDateLocationRow" class="clsAllowOptions">
         		<td class="{$myobj->getCSSFormLabelCellClass('music_category_type')}"><label for="music_category_type1">{$LANG.managemusiccategory_music_category_type}</label></td>
          		<td class="{$myobj->getCSSFormFieldCellClass('music_category_type')}">
					<input type="radio" class="clsCheckRadio" name="music_category_type" id="music_category_type1" value="Porn" tabindex="{smartyTabIndex}"{$myobj->isCheckedRadio('music_category_type','Porn')} />&nbsp;<label for="music_category_type1" >{$LANG.porn}</label>&nbsp;&nbsp;
					<input type="radio" class="clsCheckRadio" name="music_category_type" id="music_category_type2" value="General" tabindex="{smartyTabIndex}"{$myobj->isCheckedRadio('music_category_type','General')}/>&nbsp;<label for="music_category_type2">{$LANG.general}</label></td>
					<p>{$myobj->getFormFieldErrorTip('music_category_type')}</p>
        	</tr>
		{/if}
			<tr>
...
<p class="clsPageLink" >{$LANG.managemusicsubcategorytitle}</p>
	...
	<div id="selCreateSubCategory">
		...
		<table class="clsNoBorder" summary="{$LANG.managemusiccategory_create_tbl_summary}">
            ...
            <tr>
				...
				<td class="{$myobj->getCSSFormFieldCellClass('sub_category_image')}">
					{if $myobj->getFormField('sub_category') && $myobj->getFormField(music_category_ext)!=''}
							<img src="{$myobj->sub_category_image}"/>

					{/if}

				{if $myobj->getFormField(music_category_ext)=='' && $myobj->getFormField('sub_category_id')}
									<img src="{$CFG.site.url}/design/templates/{$CFG.html.template.default}/admin/images/{$CFG.html.stylesheet.screen.default}/no_image/noImage_audio_T.jpg" />
		...
		<input type="hidden" id="music_category_ext" name="music_category_ext" value="{$myobj->getFormField('music_category_ext')}" />
...
<!--                <th>{$LANG.managemusiccategory_category}</th>-->
      	...
      	{foreach item=pscValue from=$populateSubCategories_arr.row}
        	<tr>
                <td class="clsAlignCenter clsSelectAllItems"><input type="checkbox" class="clsCheckRadio" name="category_ids[]" value="{$pscValue.record.music_category_id}" tabindex="{smartyTabIndex}" {$pscValue.checked} onClick="disableHeading('selFormSubCategory');"/></td>
                <td>
                    <p id="categoryName">{$pscValue.record.music_category_name}</p>
                </td>
<!--                <td>{$pscValue.record.music_category_name}</td>-->
                <td class="clsSmallWidth">{$pscValue.record.date_added}</td>
                <td class="clsSmallWidth"><p id="edit"><a href="manageMusicCategory.php?category_id={$myobj->getFormField('category_id')}&amp;sub_category_id={$pscValue.record.music_category_id}&amp;opt=subedit">{$LANG.managemusiccategory_edit}</a></p></td>
            </tr>
        {/foreach}
            <tr>
                <td colspan="4">
        ...
        {if $myobj->isResultsFound()}
            ...
            <table summary="{$LANG.managemusiccategory_tbl_summary}">
                <tr>
                    ...
                    <th>{$LANG.managemusiccategory_category_id}</th>
                    <th>{$LANG.managemusiccategory_category}</th>
                    <th>{$LANG.managemusiccategory_music_category_type}</th>
                    <th>{$LANG.managemusiccategory_music_count}</th>
                    <th>{$LANG.managemusiccategory_description}</th>
                    <th>{$LANG.managemusiccategory_allow_post}</th>
			...
			{foreach key=scKey item=scValue from=$showCategories_arr}
                <tr>
                    <td class="clsAlignCenter"><input type="checkbox" class="clsCheckRadio" name="category_ids[]" value="{$scValue.record.music_category_id}" tabindex="{smartyTabIndex}" {$scValue.checked} onClick="checkCheckBox(this.form, 'check_all');"/>
					</td>
                    <td>{$scValue.record.music_category_id}</td>
                    <td>
                        <p id="categoryName">{$scValue.record.music_category_name}</p>

                    </td>
                    <td>{$scValue.record.music_category_type}</td>
                    <td>{$myobj->getmusicCount($scValue.record.music_category_id)}</td>

                    <td>{$scValue.record.music_category_description}</td>
                    <td>{$scValue.record.allow_post}</td>
                    <td>
                        <p>{$scValue.record.music_category_status}</p>
                    </td>
                    <td>
                        <p>{$scValue.record.date_added}</p>
                    </td>
                    <td>
                        <p id="edit"><a href="manageMusicCategory.php?category_id={$scValue.record.music_category_id}&amp;start={$myobj->getFormField('start')}">{$LANG.managemusiccategory_edit}</a>
						</p>
						<p>
						<a href="manageMusicCategory.php?category_id={$scValue.record.music_category_id}&amp;opt=sub">{$LANG.managemusicsubgenrelink}</a>
						</p>
                    </td>
                </tr>
            {/foreach}
