		...
		{if $myobj->isResultsFound()}
            ...
            <div>
                <form name="form_comments_list" id="form_comments_list" method="post" action="{$myobj->getCurrentUrl()}" autocomplete="off">
                    ...
                    <table summary="{$LANG.tbl_summary}" class="clsFormTableSection">
                        <tr>
                            <th class=""><input type="checkbox" class="clsCheckRadio" name="check_all" onclick="CheckAll(document.form_comments_list.name, document.form_comments_list.check_all.name)" tabindex="{smartyTabIndex}" /></th>
                            <th>{$LANG.th_user}</th>
                            <th>{$LANG.th_comment}</th>
                            <th>{$LANG.th_date_added}</th>
                            <th>{$LANG.manageplaylistcomments_status_label}</th>
                            <th>
...
&nbsp;                            </th>
                        ...
                        {literal}
							<script language="javascript">
                                ...
                                comment_id[{/literal}{$count}{literal}] = {/literal}'{$pcValue.record.playlist_comment_id}'{literal};
                            </script>
                         {/literal}    
                            <tr class="{$myobj->getCSSRowClass()}">
                                <td class="clsSelectAllItems"><input type="checkbox" class="clsCheckRadio" name="cid[]" value="{$pcValue.record.playlist_comment_id}" tabindex="{smartyTabIndex}" onClick="disableHeading('form_comments_list');"/></td>
                                ...
                                <td><textarea rows="4" cols="50" id="commentText_{$pcValue.record.playlist_comment_id}" name="commentText_{$pcValue.record.playlist_comment_id}"  wrap="on">{$pcValue.record.comment}</textarea></td>
                                <td>{$pcValue.record.date_added}</td>
                                <td>
                                	{if $pcValue.record.comment_status == 'Yes'}
                                		{$LANG.manageplaylistcomments_show_label}
                                    {elseif $pcValue.record.comment_status == 'No'}
                                    	{$LANG.manageplaylistcomments_hide_label}
                                    {/if}   
                                </td>
                        ...
                        {/foreach}                        
                        <tr>
                            <td colspan="6"><input type="submit" class="clsSubmitButton" name="updateSubmit" id="updateSubmit" value="{$LANG.update}" tabindex="{smartyTabIndex}" onClick="changeSubmitText({$pcValue.record.playlist_comment_id})" />
                                {$myobj->populateHidden($myobj->comments_list.hidden_arr)}
                                <a href="#" id="{$myobj->anchor}"></a>
                                <input type="button" class="clsSubmitButton" name="delete_submit" id="delete_submit" tabindex="{smartyTabIndex}" value="{$LANG.delete}" onClick="if(getMultiCheckBoxValue('form_comments_list', 'check_all', '{$LANG.check_atleast_one}')) {literal} { {/literal} Confirmation('selMsgConfirmDelete', 'deleteForm', Array('cid', 'act', 'confirmation_msg'), Array(multiCheckValue, 'delete', '{$LANG.delete_confirmation|nl2br}'), Array('value', 'value', 'innerHTML'), -100, -500); {literal} } {/literal}" />                            </td>
                        </tr>
                    </table>
              </form>
            </div>
