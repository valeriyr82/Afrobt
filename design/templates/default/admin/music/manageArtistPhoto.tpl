            ...
            {if $myobj->isResultsFound()}
                ...
                <table>
                    ...
                    {foreach key=musicArtistListKey item=musicArtistList from=$myobj->list_artistlist_block.showArtistlists.row}
                        <tr>
                        	<td class="clsSmallWidth">{$musicArtistList.record.music_artist_id}
                            </td>
                            ...
                            {if $musicArtistList.artist_path  != ''}
                            	<img   src="{$musicArtistList.artist_path}"{$musicArtistList.disp_image} title="{$musicArtistList.record.artist_name}"/> 
                            {else}   
       	...
       	{if $myobj->isResultsFound()}
            ...
            <table >
                <tr>
                    ...
                    <th>{$LANG.manageartistphoto_photo_mainphoto}</th>
            ...
            {foreach key=asKey item=saValue from=$myobj->list_photo_block.showArtistImageList}
                <tr>
                    <td class="clsSelectAllItems"><input type="checkbox" class="clsCheckRadio" name="forum_ids[]" value="{$saValue.record.music_artist_image}" onClick="disableHeading('selFormArtistPhoto');" tabindex="{smartyTabIndex}"/></td>
                    <td><p>
                    ...
                    <td>{if $saValue.record.main_img == '1'}{$LANG.manageartistphoto_yes_label}{else} {$LANG.manageartistphoto_no_label}{/if}</td>
                    <td>
                    ...
                    <td>
                        <p>
	                        <a href="javascript:void(0);" onclick="return Confirmation('selMsgConfirmSingle', 'msgConfirmformSingle',Array('action', 'music_artist_image', 'artistImg', 'confirmMessageSingle'), Array('Delete', '{$saValue.record.music_artist_image}', '{$saValue.artist_image}', '{$LANG.manageartistphoto_delete_confirmation}'), Array('value', 'value', 'src', 'innerHTML'), -100, -500);">{$LANG.manageartistphoto_delete}</a>                        
                        <p>
                        {if $saValue.record.status == 'No'}
                            <p>
                                <a href="javascript:void(0);" onclick="return Confirmation('selMsgConfirmSingle', 'msgConfirmformSingle',Array('action', 'music_artist_image', 'artistImg', 'confirmMessageSingle'), Array('Active', '{$saValue.record.music_artist_image}', '{$saValue.artist_image}', '{$LANG.manageartistphoto_active_confirm_message}'), Array('value', 'value', 'src', 'innerHTML'), -100, -500);">{$LANG.common_display_active}</a>                        
                            <p>
                        {elseif $saValue.record.status == 'Yes'}
                            <p>
                                <a href="javascript:void(0);" onclick="return Confirmation('selMsgConfirmSingle', 'msgConfirmformSingle',Array('action', 'music_artist_image', 'artistImg', 'confirmMessageSingle'), Array('Inactive', '{$saValue.record.music_artist_image}', '{$saValue.artist_image}', '{$LANG.manageartistphoto_inactive_confirm_message}'), Array('value', 'value', 'src', 'innerHTML'), -100, -500);">{$LANG.common_display_inactive}</a>                        
                            <p>
                        {elseif $saValue.record.status == 'ToActivate'}    
                            <p>
                                <a href="javascript:void(0);" onclick="return Confirmation('selMsgConfirmSingle', 'msgConfirmformSingle',Array('action', 'music_artist_image', 'artistImg', 'confirmMessageSingle'), Array('Approve', '{$saValue.record.music_artist_image}', '{$saValue.artist_image}', '{$LANG.manageartistphoto_approve_confirm_message}'), Array('value', 'value', 'src', 'innerHTML'), -100, -500);">{$LANG.manageartistphoto_photo_approve}</a> /                       
                              	<a href="javascript:void(0);" onclick="return Confirmation('selMsgConfirmSingle', 'msgConfirmformSingle',Array('action', 'music_artist_image', 'artistImg', 'confirmMessageSingle'), Array('Delete', '{$saValue.record.music_artist_image}', '{$saValue.artist_image}', '{$LANG.manageartistphoto_disapprove_confirm_message}'), Array('value', 'value', 'src', 'innerHTML'), -100, -500);">{$LANG.manageartistphoto_photo_disapprove}</a>                          
                            <p>
                        {/if}
                        {if $saValue.record.main_img}	
                        	<p>	
                            	<a href="javascript:void(0);" onclick="return Confirmation('selMsgConfirmSingle', 'msgConfirmformSingle',Array('action', 'music_artist_image', 'artistImg', 'confirmMessageSingle'), Array('remove_main_img', '{$saValue.record.music_artist_image}', '{$saValue.artist_image}', '{$LANG.manageartist_confirm_remove_lyrics}'), Array('value', 'value', 'src', 'innerHTML'), -100, -500);">{$LANG.managelyrics_remove_main_photo}</a>                           </p> 
                         {else}   
                         	<p>	
                            	<a href="javascript:void(0);" onclick="return Confirmation('selMsgConfirmSingle', 'msgConfirmformSingle',Array('action', 'music_artist_image', 'artistImg', 'confirmMessageSingle'), Array('main_img', '{$saValue.record.music_artist_image}', '{$saValue.artist_image}', '{$LANG.manageartistphoto_confirm_main_image}'), Array('value', 'value', 'src', 'innerHTML'), -100, -500);">{$LANG.manageartistphoto_main_image}</a>                           </p>             	
                         {/if}					</td>
                </tr>
            {/foreach}  
             <tr>
                    <td colspan="6">
                        ...
                        <input type="button" class="clsCancelButton" name="cancel_submit" id="cancel_submit" value="{$LANG.manageartistphoto_cancel_label}" onclick="Redirect2URL('{$CFG.site.url}admin/music/manageArtistPhoto.php')"tabindex="{smartyTabIndex}">                        </td>
              </tr>   
             </table>   
  </form>
