            ...
            {if isset($opt)}
             ...
             <div class="clsLoginFormTable">
            	<table class="clsTwoColumnTbl">
					<tr>
						<td class="{$myobj->getCSSFormLabelCellClass('user_name')}"><span class="clsMandatoryFieldIcon">{$LANG.common_mandatory_field_icon}</span><label for="head_user_name">{$myobj->form_login.login_field_label}</label></td>
						<td class="{$myobj->getCSSFormFieldCellClass('user_name')}">
							{if !isset($opt)}
								<input type="text" class="clsTextBox" name="head_user_name" id="head_user_name" tabindex="{smartyTabIndex}" value="{$myobj->getFormField('user_name')}" maxlength="{$CFG.fieldsize.username.max}" />
							{else}
								<input type="text" class="clsTextBox" name="head_user_name" id="head_user_name" tabindex="800" value="{$myobj->getFormField('user_name')}" maxlength="{$CFG.fieldsize.username.max}" />
							{/if}
					...
					<tr>
						<td class="{$myobj->getCSSFormLabelCellClass('password')}"><span class="clsMandatoryFieldIcon">{$LANG.common_mandatory_field_icon}</span><label for="head_password">{$LANG.common_password}</label></td>
						<td class="{$myobj->getCSSFormFieldCellClass('password')}">
							{if !isset($opt)}
								<input type="password" class="clsTextBox" name="head_password" id="head_password" tabindex="{smartyTabIndex}" value="{$myobj->getFormField('head_password')}" maxlength="{$CFG.fieldsize.password.max}" />
							{else}
								<input type="password" class="clsTextBox" name="head_password" id="head_password" tabindex="805" value="{$myobj->getFormField('password')}" maxlength="{$CFG.fieldsize.password.max}" />
							{/if}
