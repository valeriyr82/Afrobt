		 ...
		 */
		    public function populateMusicJsVars()
			{
				echo '<script type="text/javascript">
				      var play_songs_playlist_player_url = "'.$this->playSongsUrl.'";
				...
				echo '
                var volume_help_tip_top_pos = '.$this->CFG['admin'][$this->CFG['html']['template']['default']]['musics']['volume_helptip_info_top'].';';
				echo '
                var volume_help_tip_left_pos = '.$this->CFG['admin'][$this->CFG['html']['template']['default']]['musics']['volume_helptip_info_left'].';';
				echo '
                var volume_iframe_help_tip_top_pos = '.$this->CFG['admin'][$this->CFG['html']['template']['default']]['musics']['volume_iframe_helptip_info_top'].';';
				echo '
                var volume_iframe_help_tip_left_pos = '.$this->CFG['admin'][$this->CFG['html']['template']['default']]['musics']['volume_iframe_helptip_info_left'].';';
				...
				foreach($this->quick_mix_id_arr as $quick_mix)
					{
						echo '
                        quick_mix_music_id_arr.push('.$quick_mix.');';
					}
					echo '
                    quickMixPopupWindow = eval('.$this->quick_mix_window_name.');';
				echo '
                </script>';
			}
...
<?php
		...
		public function populateMusicAlbum($type='Public', $user_id='', $album_id)
		{
			...
			while($row = $rs->FetchRow())
			{
				...
				?><option value="<?php echo $row['music_album_id'];?>" <?php echo $selected;?>><?php echo $out;?></option><?
				$inc++;
			}
		}
