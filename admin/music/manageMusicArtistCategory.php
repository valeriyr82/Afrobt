...
$CFG['lang']['include_files'][] = 'languages/%s/music/admin/manageMusicArtistCategory.php';
...
class MusicCategoryFormHandler extends MusicHandler
	{
		...
		public function storeImagesTempServer($uploadUrl, $extern)
			{
				...
				if($this->CFG['admin']['musics']['artist_category_height'] or $this->CFG['admin']['musics']['artist_category_width'])
					{
						$this->imageObj->resize($this->CFG['admin']['musics']['artist_category_width'], $this->CFG['admin']['musics']['artist_category_height'], '-');
		...
		*/
		public function isValidCategoryId($category_id, $err_tip='')
			{
				$sql = 'SELECT music_artist_category_id, music_artist_category_description,'.
				' music_artist_category_name, music_artist_category_status, date_added, music_artist_category_type, music_artist_category_ext, priority'.
				' FROM '.$this->CFG['db']['tbl']['music_artist_category'].
				' WHERE music_artist_category_id = '.$this->dbObj->Param($this->fields_arr[$category_id]);
		...
		*/
		public function chkmusicExists($category_ids)
			{
				$sql = 'SELECT count( g.user_id ) AS cat_count, music_artist_category_id'.' FROM '.$this->CFG['db']['tbl']['users'].' g'.
				' WHERE music_artist_category_id IN ( '.$category_ids.' )'.' AND usr_status!=\'Deleted\''.' GROUP BY music_artist_category_id';
				$stmt = $this->dbObj->Prepare($sql);
		...
		public function deleteSelectedCategories()
			{
				...
				return false;
				$this->adjustPriorityOrder(0, '', 'music_artist_category', 'music_artist_category_id', 'delete', explode(',', $category_ids));
				$sql = 'DELETE FROM '.$this->CFG['db']['tbl']['music_artist_category'].' '.
				'WHERE music_artist_category_id IN ('.$category_ids.') ';
				$stmt = $this->dbObj->Prepare($sql);
				$rs = $this->dbObj->Execute($stmt);
				if (!$rs)
				trigger_error($this->dbObj->ErrorNo().' '.$this->dbObj->ErrorMsg(), E_USER_ERROR);
		...
		*/
		public function chkCategoryExits($category, $err_tip='')
			{
				$sql = 'SELECT COUNT(music_artist_category_id) AS count FROM '.$this->CFG['db']['tbl']['music_artist_category'].' '.
				'WHERE UCASE(music_artist_category_name) = UCASE('.$this->dbObj->Param($this->fields_arr[$category]).') ';
				$fields_value_arr[] = $this->fields_arr[$category];
				if ($this->fields_arr['category_id'])
					{
						$sql .= ' AND music_artist_category_id != '.$this->dbObj->Param($this->fields_arr['category_id']);
		...
		*/
		public function isValidSubCategoryId($category_id, $err_tip='')
			{
				$sql = 'SELECT music_artist_category_id, music_artist_category_description,'.' music_artist_category_name, music_artist_category_status, date_added, music_artist_category_type, music_artist_category_ext, priority'.
				' FROM '.$this->CFG['db']['tbl']['music_artist_category'].' WHERE music_artist_category_id = '.$this->dbObj->Param($this->fields_arr[$category_id]);
				$stmt = $this->dbObj->Prepare($sql);
				$rs = $this->dbObj->Execute($stmt, array($this->fields_arr[$category_id]));
				if (!$rs)
				trigger_error($this->dbObj->ErrorNo().' '.$this->dbObj->ErrorMsg(), E_USER_ERROR);
		...
		public function createCategory($category_table)
			{
				...
				if ($this->fields_arr['category_id'])
					{
						$priority = $this->getPriorityOrder($this->fields_arr['priority'], 'music_artist_category', 'update', 0);
						$this->adjustPriorityOrder(0, $priority, 'music_artist_category', 'music_artist_category_id', 'update', $this->fields_arr['category_id']);
						$sql = 'UPDATE '.$category_table.' SET '.
						'music_artist_category_name = '.$this->dbObj->Param($this->fields_arr['category']).', '.
						'music_artist_category_description = '.$this->dbObj->Param($this->fields_arr['category_description']).', '.
						'music_artist_category_status = '.$this->dbObj->Param($this->fields_arr['status']).', '.
						'music_artist_category_type = '.$this->dbObj->Param($this->fields_arr['music_artist_category_type']).', '.
						...
						$this->fields_arr['music_artist_category_type'],
						$priority,
					...
					{
						$priority = $this->getPriorityOrder($this->fields_arr['priority'], 'music_artist_category', 'add', 0);
						$this->adjustPriorityOrder(0, $priority, 'music_artist_category', 'music_artist_category_id', 'add');
		...
		*/
		public function getmusicCount($category_id)
			{
				$sql = 'SELECT count(g.user_id) as cat_count '.
				'FROM '.$this->CFG['db']['tbl']['users'].' as g '.
				'WHERE music_artist_category_id = '.$this->dbObj->Param($category_id);
				$stmt = $this->dbObj->Prepare($sql);
		...
		public function showCategories()
			{
				...
				while($row = $this->fetchResultRecord())
					{
						$row['music_artist_category_name'] =  wordWrap_mb_ManualWithSpace($row['music_artist_category_name'], $this->CFG['admin']['musics']['admin_music_category_name_length'], $this->CFG['admin']['musics']['admin_music_category_name_total_length'], 0);
						$row['music_artist_category_description'] = wordWrap_mb_ManualWithSpace($row['music_artist_category_description'], $this->CFG['admin']['musics']['admin_music_category_description_length'], $this->CFG['admin']['musics']['admin_music_category_description_total_length'], 0);
						$showCategories_arr[$inc]['record'] = $row;
						$showCategories_arr[$inc]['checked'] = '';
						if((is_array($this->fields_arr['category_ids'])) && (in_array($row['music_artist_category_id'], $this->fields_arr['category_ids'])))
		...
		public function populateSubCategories()
			{
				...
				$sql = 'SELECT music_artist_category_id, music_artist_category_name,music_artist_category_ext,'.
				...
				if($rs->PO_RecordCount())
					{
						...
						while($row = $rs->FetchRow())
							{
								$row['music_artist_category_name'] = wordWrap_mb_ManualWithSpace($row['music_artist_category_name'], $this->CFG['admin']['musics']['admin_music_category_name_length']);
		...
		public function chkValidFileType($field_name, $err_tip = '')
			{
				...
				if($extern!='')
					{
						if (!in_array($extern, $this->CFG['admin']['musics']['artist_category_image_format_arr']))
								{
		...
		public function chkValideFileSize($field_name, $err_tip='')
			{
				...
				if($extern!='')
					{
						$max_size = $this->CFG['admin']['musics']['artist_category_image_max_size'] * 1024;
		...
		public function resetFieldsArray()
			{
				...
				$this->setFormField('music_artist_category_type', 'General');
				$this->setFormField('music_artist_category_ext', '');
		...
		*/
		public function changeStatus($status)
			{
				$category_ids = $this->fields_arr['category_ids'];
				$sql = 'UPDATE '.$this->CFG['db']['tbl']['music_artist_category'].' SET'.
				' music_artist_category_status='.$this->dbObj->Param('music_artist_category_status').
				' WHERE music_artist_category_id IN('.$category_ids.')';
				$stmt = $this->dbObj->Prepare($sql);
		...
		*/
		public function chkIsDuplicateSubCategory($err_tip = '')
			{
				$sql = 'SELECT music_artist_category_id FROM '.$this->CFG['db']['tbl']['music_artist_category'].
				' WHERE music_artist_category_name='.$this->dbObj->Param('music_artist_category_name').
		...
		public function chkIsDuplicateSubCategoryForEdit($err_tip = '')
			{
				...
				' WHERE music_artist_category_name='.$this->dbObj->Param('music_artist_category_name').
				' AND parent_category_id='.$this->dbObj->Param('parent_category_id').
				' AND music_artist_category_id!='.$this->dbObj->Param('music_artist_category_id');
				$stmt = $this->dbObj->Prepare($sql);
		...
		*/
		public function addSubCategory()
			{
				$priority = $this->getPriorityOrder($this->fields_arr['priority'], 'music_artist_category', 'add', $this->fields_arr['category_id']);
				$this->adjustPriorityOrder($this->fields_arr['category_id'], $priority, 'music_artist_category', 'music_artist_category_id', 'add');
				$sql = 'INSERT INTO '.$this->CFG['db']['tbl']['music_artist_category'].' SET'.
				' music_artist_category_name='.$this->dbObj->Param('music_artist_category_name').','.
				' parent_category_id='.$this->dbObj->Param('parent_category_id').','.
				' priority='.$this->dbObj->Param('priority').','.
				' music_artist_category_status=\'Yes\','.
				' date_added=NOW()';
				$stmt = $this->dbObj->Prepare($sql);
		...
		*/
		public function updateSubCategory()
			{
				$priority = $this->getPriorityOrder($this->fields_arr['priority'], 'music_artist_category', 'update', $this->fields_arr['category_id']);
				$this->adjustPriorityOrder($this->fields_arr['category_id'], $priority, 'music_artist_category', 'music_artist_category_id', 'update', $this->fields_arr['category_id']);
				$sql = 'UPDATE '.$this->CFG['db']['tbl']['music_artist_category'].' SET'.
				' music_artist_category_name='.$this->dbObj->Param('music_artist_category_name').', '.
				' priority='.$this->dbObj->Param('priority').' WHERE'.
				' music_artist_category_id='.$this->dbObj->Param('music_artist_category_id').' AND'.
				' parent_category_id='.$this->dbObj->Param('parent_category_id');
				$stmt = $this->dbObj->Prepare($sql);
		...
		*/
		public function populateSubCategory()
			{
				$sql = 'SELECT music_artist_category_name, priority, music_artist_category_id,music_artist_category_ext  FROM '.$this->CFG['db']['tbl']['music_artist_category'].
				' WHERE music_artist_category_id='.$this->dbObj->Param('music_artist_category_id').
				' AND parent_category_id='.$this->dbObj->Param('parent_category_id');
				$stmt = $this->dbObj->Prepare($sql);
				...
				if($row = $rs->FetchRow())
					{
						$this->fields_arr['sub_category'] = $row['music_artist_category_name'];
						$this->fields_arr['priority'] = $row['priority'];
						$this->fields_arr['sub_category_id'] = $row['music_artist_category_id'];
						$this->fields_arr['music_artist_category_ext'] = $row['music_artist_category_ext'];
						return true;
					}
				return $row['music_artist_category_ext'];
			}
		/**
		...
		public function deleteSeletctedSubCategories()
			{
				...
				$category_ids = $this->fields_arr['category_ids'];
				$this->adjustPriorityOrder($category_id, '', 'music_artist_category', 'music_artist_category_id', 'delete', explode(',', $category_ids));
				$sql = 'DELETE FROM '.$this->CFG['db']['tbl']['music_artist_category'].
				' WHERE music_artist_category_id  IN('.$this->fields_arr['category_ids'].')';
				$stmt = $this->dbObj->Prepare($sql);
				$rs = $this->dbObj->Execute($stmt);
				if (!$rs)
				trigger_error($this->dbObj->ErrorNo().' '.$this->dbObj->ErrorMsg(), E_USER_ERROR);
		...
		**/
		public function updateCategoryImageExt($categoryId,$category_ext)
			{
				$sql = 'UPDATE '.$this->CFG['db']['tbl']['music_artist_category'].' SET '.
				'music_artist_category_ext = '.$this->dbObj->Param($category_ext).' '.
				'WHERE music_artist_category_id = '.$this->dbObj->Param($this->category_id);
				$stmt = $this->dbObj->Prepare($sql);
...
$category->setFormField('asc', 'gc.music_artist_category_name');
$category->setFormField('dsc', '');
$category->setFormField('priority', '');
$category->setFormField('sub_category_id', '');
$category->setFormField('music_artist_category_ext', '');
$category->imageFormat=implode(', ',$CFG['admin']['musics']['artist_category_image_format_arr']);
//$category->setFormField('category_id', '');
/*************Start navigation******/
//fetch number of records show in the result--numpg
$condition = '';
//Set tables and fields to return
$category->setTableNames(array($CFG['db']['tbl']['music_artist_category'].' as gc'));
$category->setReturnColumns(array('gc.music_artist_category_id, gc.priority,gc.music_artist_category_description, gc.music_artist_category_name, gc.music_artist_category_status, DATE_FORMAT(gc.date_added, \''.$CFG['format']['date'].'\') as date_added','music_artist_category_ext','music_artist_category_type'));
		...
		if ($category->isValidFormInputs())
			{
				if($category->getFormField('opt')=='sub')
				...
				else
					{
						...
						$category->setAllPageBlocksHide();
						$category->setFormField('category_id', $category->category_details_arr['music_artist_category_id']);
						$image_extern=$category->category_details_arr['music_artist_category_ext'];
						$category->category_image_ext_music = $category->category_details_arr['music_artist_category_ext'];
						$category->category_tmp_music_image = $category->media_relative_path.$CFG['admin']['musics']['artist_category_folder'].$CFG['admin']['musics']['music_no_image'];
						$category->category_image = $category->media_relative_path.$CFG['admin']['musics']['artist_category_folder'].$category->category_details_arr['music_artist_category_id'].'.'.$category->category_details_arr['music_artist_category_ext'];
						$category->setFormField('category', stripslashes($category->category_details_arr['music_artist_category_name']));
						$category->setFormField('category_description', stripslashes($category->category_details_arr['music_artist_category_description']));
						$category->setFormField('status', $category->category_details_arr['music_artist_category_status']);
						$category->setFormField('music_artist_category_type', $category->category_details_arr['music_artist_category_type']);
						$category->setFormField('priority', $category->category_details_arr['priority']);
						$category->setFormField('music_artist_category_ext', $category->category_details_arr['music_artist_category_ext']);
					}
				if($category->getFormField('opt')=='subedit')
					{
						$category->category_image = $category->media_relative_path.$CFG['admin']['musics']['artist_category_folder'].$category->getFormField('sub_category_id').'.'.$category->category_details_arr['music_artist_category_ext'];
					}
			}
		else
			{
...
if ($category->isFormPOSTed($_POST, 'sub_category_submit'))
					...
					if ($category->isValidFormInputs() && isset($_FILES['sub_category_image']['name']) && trim($_FILES['sub_category_image']['name']!=''))
						{
							...
							if (in_array($extern, $CFG['admin']['musics']['artist_category_image_format_arr']))
								{
									if ($_FILES['sub_category_image']['size'] <= $CFG['admin']['musics']['artist_category_image_max_size'] * 1024)
										{
											if (!$_FILES['sub_category_image']['error'])
												{
													...
													$temp_dir = $category->media_relative_path.$CFG['admin']['musics']['artist_category_folder'];
...
else if($category->chkIsEditModeSub())
	{
		if($category->isFormPOSTed($_POST, 'update_category_submit'))
			...
			{
				$category->setPageBlockShow('form_create_sub_category');
				$category->sub_category_image = $category->media_relative_path.$CFG['admin']['musics']['artist_category_folder'].$category->getFormField('sub_category_id').'.'.$category->getFormField('music_artist_category_ext');
			}
	}
...
else if ($category->isFormPOSTed($_POST, 'category_submit'))
	{
		...
		$category->createCategory($CFG['db']['tbl']['music_artist_category']);
		if ($category->isValidFormInputs() && isset($_FILES['category_image']['name']) && trim($_FILES['category_image']['name']!=''))
			{
				...
				if (in_array($extern, $CFG['admin']['musics']['artist_category_image_format_arr']))
					{
						if ($_FILES['category_image']['size'] <= $CFG['admin']['musics']['artist_category_image_max_size'] * 1024)
							{
...
if ($category->isFormPOSTed($_POST, 'sub_category_cancel'))
	{
		$url = $CFG['site']['url'].'admin/music/manageMusicArtistCategory.php?category_id='.$category->getFormField('category_id').'&opt=sub';
		$category->resetFieldsArray();
		Redirect2URL($url);
	}
/*************Start navigation******/
if (!$category->isShowPageBlock('form_create_sub_category'))
	{
		...
		$category->checkSortQuery('gc.music_artist_category_name', 'asc');
		$category->buildQuery();
...
$smartyObj->display('manageMusicArtistCategory.tpl');
