...
/**
 * To edit the config_music file
 *
 * TO show the config_music variables from the file and edit
 *
 * PHP version 5.0
 *
 * @category	Cann
 ...
 * @version		SVN: $Id: editConfigMusic.php
 * @since 		2006-10-16
 */
...
$CFG['lang']['include_files'][] = 'languages/%s/music/admin/editConfigMusic.php';
...
<?php
		...
		public function chkFileNmaeIsNotEmpty($field_name, $err_tip = '')
			...
			/**
			 * ConfigProfileEditFormHandler::insertPreviewStartCron()
			 *
			 * @return
			 *
			 */
			public function insertPreviewStartCron()
			{
				...
				$sql = 'INSERT INTO '.$this->CFG['db']['tbl']['trim_music_cron'].
						' SET preview_start='.$this->dbObj->Param('preview_start').','.
						' preview_length='.$this->dbObj->Param('preview_length').','.
						' date_added=NOW()';
				$stmt = $this->dbObj->Prepare($sql);
				$rs = $this->dbObj->Execute($stmt, array($_POST['trim_music_start_position'], $_POST['trim_music_duration']));
				if (!$rs)
					trigger_error($this->dbObj->ErrorNo().' '.
					$this->dbObj->ErrorMsg(), E_USER_ERROR);
			}
...
$CFG['config_music_path'] = '../../common/configs/config_music.inc.php';
...
$full_length_music_arr = array('All'=>$LANG['allow_all'], 'members'=>$LANG['registered_users'],'paid_members'=>$LANG['paid_members']);
$download_previlages_arr = array('All'=>$LANG['allow_all'], 'members'=>$LANG['registered_users'],'paid_members'=>$LANG['paid_members']);

//ADDED THE ARRAY FOR ALLOWED TYPE TO UPLOAD THE MUSIC
$sale_previlages_arr = array(
	'All'=>$LANG['allow_all'],
	'Artist'=>$LANG['artist_members'],
	'None'=>$LANG['disallow_all']
);

$youtube_download_format_arr = array('5'=>$LANG['format_normal'], '35'=>$LANG['format_HD_640x480'],'22'=>$LANG['format_HD_1280x720']);
...
$editfilefrm->getFileContent($CFG['config_music_path']);
//Declared the config log file name
$editfilefrm->setFormfield('config_file_name', 'config_music');
				...
				while($is_repeat)
					{
						...
						if ($start_of_comment !== false and
								$end_of_comment !== false)
							{
								...
								//check if the new value is different from the older value and inserted in to table
								if((isset($tags['@cfg_key']) and $tags['@cfg_key']=='trim_music_duration')
									 OR (isset($tags['@cfg_key']) and $tags['@cfg_key']=='trim_music_start_position'))
								{
									if(($CFG['admin']['musics']['trim_music_duration']!=$_POST['trim_music_duration'])
										OR ($CFG['admin']['musics']['trim_music_start_position']!=$_POST['trim_music_start_position']))
									{
										$editfilefrm->insertPreviewStartCron();
									}
								}
							...
							else
								{
									...
									//Start to copy the old config file to config log
									$format="%d-%m-%Y_%H-%M-%S";
									$strf=strftime($format);
									$file = $CFG['config_music_path'];
									...
									$fw = fopen($CFG['config_music_path'], 'w');
...
$editfilefrm->left_navigation_div = 'musicMain';
//<<<<<---------------------Code ends------------------//
...
<script language="javascript" type="text/javascript">
	...
	function chkValidFileFormat(field_name, format)
		{
			var str = $Jq.trim($Jq('#' + field_name).val());
	...
	function chkMandatoryFields()
		{
			if($Jq.trim($Jq('#LogoUrl').val())=='')
				return true;

			if($Jq.trim($Jq('#TopUrlUrl').val())=='')
				return true;
...
if ($editfilefrm->isShowPageBlock('show_config_variable'))
	{
		...
		$config_content = '';
		$ret_array = array();
		$config_content  = file_get_contents($CFG['config_music_path']);
...
?>
					<tr>
						<td colspan="2"><b><?php echo $value['tags']['@cfg_sub_head']?></b></td>
					</tr>
<?php
						}
					switch ($variable_data_type)
...
<?php
											break;
										//Added the new case for Sale Privileges
										case 'allowed_usertypes_to_upload_for_sale':
?>
		<tr>
			<td class="clsFormLabelCellDefault"><?php $editfilefrm->renderLabelTableCell($value['tags']['@cfg_key'], $value['tags']['@cfg_label']); ?></td>
			<td class="clsFormFieldCellDefault">
				<select name="allowed_usertypes_to_upload_for_sale" id="allowed_usertypes_to_upload_for_sale" tabindex="<?php echo $tab_index;?>">
					<?php $editfilefrm->populateArray($sale_previlages_arr, $value['value']);?>
				</select>
			</td>
		</tr>
<?php
										...
										case 'full_length_music':
?>
		<tr>
			<td class="clsFormLabelCellDefault"><?php $editfilefrm->renderLabelTableCell($value['tags']['@cfg_key'], $value['tags']['@cfg_label']); ?></td>
			<td class="clsFormFieldCellDefault">
				<select name="full_length_music" id="full_length_music" tabindex="<?php echo $tab_index;?>">
					<?php $editfilefrm->populateArray($full_length_music_arr, $value['value']);?>
				</select>
			</td>
		</tr>
<?php
...
<?php
											break;
											case 'youtube_download_format':
?>
		<tr>
			<td class="clsFormLabelCellDefault"><?php $editfilefrm->renderLabelTableCell($value['tags']['@cfg_key'], $value['tags']['@cfg_label']); ?></td>
			<td class="clsFormFieldCellDefault">
				<select name="youtube_download_format" id="youtube_download_format" tabindex="<?php echo $tab_index;?>">
					<?php $editfilefrm->populateArray($youtube_download_format_arr, $value['value']);?>
				</select>
			</td>
		</tr>
<?php
...
<?php
							...
							case 'int' :
								...
								switch($value['tags']['@cfg_key'])
								{
									case 'trim_music_duration':
?>
		<tr>
			<td class="clsFormLabelCellDefault"><?php $editfilefrm->renderLabelTableCell($value['tags']['@cfg_key'], $value['tags']['@cfg_label']); ?></td>
			<td class="clsFormFieldCellDefault"><input type="text" name="trim_music_duration" onChange="javascript:changeTrimAlert(this.value)"  value="<?php echo $value['value'];?>">
			</td>
		</tr>
<?php
										break;
									case 'trim_music_start_position':
?>
		<tr>
			<td class="clsFormLabelCellDefault"><?php $editfilefrm->renderLabelTableCell($value['tags']['@cfg_key'], $value['tags']['@cfg_label']); ?></td>
			<td class="clsFormFieldCellDefault"><input type="text" name="trim_music_start_position" onChange="javascript:changeTrimAlertStart(this.value)"  value="<?php echo $value['value'];?>">
			</td>
		</tr>
<?php
										break;
...
<div id="selMsgConfirm" class="clsPopupConfirmation" style="display:none;position:absolute;">
        ...
        <form name="msgConfirmform" id="msgConfirmform" method="post" action="<?php $editfilefrm->getCurrentUrl();?>">
            <table summary="">
                <tr>
                    <td>
                        <input type="button" class="clsCancelButton" name="cancel" id="cancel" value="Ok"  onClick="return hideAllBlocks();" />
                    </td>
                </tr>
            </table>
        </form>
...
<script language="javascript" type="text/javascript" >
	...
	function changeTrimAlert(value)
	{
		var oldvalue = '<?php echo $CFG['admin']['musics']['trim_music_duration'];?>';
		if(value!=oldvalue)
		{
			Confirmation('selMsgConfirm', 'msgConfirmform', Array('confirmMessage'), Array('<?php echo $LANG['trim_length_change_alert_message'] ;?>'), Array('innerHTML'), -25, -290);
		}
		else
			return true;
	}
	function changeTrimAlertStart(value)
	{
		var oldvalue = '<?php echo $CFG['admin']['musics']['trim_music_start_position'];?>';
		if(value!=oldvalue)
		{
			Confirmation('selMsgConfirm', 'msgConfirmform', Array('confirmMessage'), Array('<?php echo $LANG['trim_length_change_alert_message'] ;?>'), Array('innerHTML'), -25, -290);
		}
