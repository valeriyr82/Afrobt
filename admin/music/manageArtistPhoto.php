...
class viewArtist extends MusicHandler
	{
		...
		public function setTableAndColumns()
			{
				...
				$this->setReturnColumns(array('mai.music_artist_image', 'mai.music_artist_id', 'mai.user_id', 'mai.image_caption', 'mai.image_ext', 'mai.main_img', 'mai.status'));
				$this->sql_condition = 'u.usr_status <> \'Deleted\' AND mai.status<>\'Deleted\' AND music_artist_id='.$this->fields_arr['artist_id'];
				...
				while($row = $this->fetchResultRecord())
					{
						...
						$showArtistImageList_arr[$inc]['artist_image_detail'] = $this->getArtistImageDetail($row['music_artist_id']);
		...
		public function setMainArtistImage()
			{
				...
				$sql = 'UPDATE '.$this->CFG['db']['tbl']['music_artist_image'].
						...
						' WHERE music_artist_image = '.$this->dbObj->Param('music_artist_image').' AND music_artist_id = '.$this->dbObj->Param('artist_id');

				$stmt = $this->dbObj->Prepare($sql);
				$rs = $this->dbObj->Execute($stmt, array('1', $this->fields_arr['music_artist_image'], $this->fields_arr['artist_id']));
				if (!$rs)
					trigger_db_error($this->dbObj);
		...
		public function changeStatus($status)
			{
				$music_artist_image = $this->fields_arr['music_artist_image'];
				$sql = 'UPDATE '.$this->CFG['db']['tbl']['music_artist_image'].
						' SET status = '.$this->dbObj->Param('status').' '.
						'WHERE music_artist_image IN ('.$music_artist_image.')' ;
				...
				while($row = $this->fetchResultRecord())
					{
						...
						$showArtistlists_arr['row'][$inc]['artist_image_detail'] = $this->getArtistImageDetail($row['music_artist_id'], 'Yes');
						if(!empty($showArtistlists_arr['row'][$inc]['artist_image_detail']))
							{
								$music_artist_image = $showArtistlists_arr['row'][$inc]['artist_image_detail']['music_artist_image'];
								$showArtistlists_arr['row'][$inc]['artist_path'] = $artist_image_path.$music_artist_image.$this->CFG['admin']['musics']['artist_thumb_name'].'.'.$showArtistlists_arr['row'][$inc]['artist_image_detail']['image_ext'];
		...
		public function getArtistPhotoStatusCount($artist_id,$status)
			{
				$sql = 'SELECT count(music_artist_image) as total FROM '.$this->CFG['db']['tbl']['music_artist_image'].
...
if ($viewArtist->isShowPageBlock('list_artistlist_block'))
	{
		...
		$viewArtist->setTableNames(array($CFG['db']['tbl']['music_artist'].' AS mai '));
		$viewArtist->setReturnColumns(array('mai.music_artist_id', 'mai.artist_name'));
		$viewArtist->setPageBlockShow('list_artistlist_block');
		if ($viewArtist->isFormPOSTed($_POST, 'search'))
			{
				$condition = 'artist_name  LIKE "%'.$viewArtist->getFormField('name').'%"';
			}
		$viewArtist->buildSelectQuery();
		$viewArtist->buildConditionQuery($condition);
		$viewArtist->checkSortQuery('mai.music_artist_id', 'DESC');
		...
		if(act_value)
			{
				...
				Confirmation('selMsgConfirm', 'msgConfirmform', Array('music_artist_image'), Array(multiCheckValue), Array('value'),'selFormForums');
			}
			else
