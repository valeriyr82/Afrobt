...
/**
 * To edit the config_blog file
 *
 * TO show the config_blog variables from the file and edit
 *
 * PHP version 5.0
 *
 * @category	Cann
 ...
 * @version		SVN: $Id: editConfigBlog.php
 * @since 		2006-10-16
 */
...
$CFG['lang']['include_files'][] = 'languages/%s/blog/admin/editConfigBlog.php';
 ...
 * <b>Class overview</b>
 *
 * This class is use to handle the editing and listing of config_blog variables
 *
 * <b>Methods overview
 *
...
$CFG['config_blog_path'] = '../../common/configs/config_blog.inc.php';
...
$watermark_image_position_arr  = array('top_right'=>$LANG['top_right'], 'top_left'=>$LANG['top_left'], 'bottom_left'=>$LANG['bottom_left'],
							'bottom_right'=>$LANG['bottom_right'], 'center'=>$LANG['center']);

//ADDED THE ARRAY FOR ALLOWED TYPE TO UPLOAD THE BLOG
...
$editfilefrm->getFileContent($CFG['config_blog_path']);
						...
						if ($start_of_comment !== false and
								$end_of_comment !== false)
							{
							...
							else
								{
									...
									$fw = fopen($CFG['config_blog_path'], 'w');
...
$editfilefrm->left_navigation_div = 'blogSetting';
//<<<<<---------------------Code ends------------------//
...
if ($editfilefrm->isShowPageBlock('show_config_variable'))
	{
		...
		$config_content = '';
		$ret_array = array();
		$config_content  = file_get_contents($CFG['config_blog_path']);
...
<?php
										...
										case 'watermark_image_position':
?>
		<tr>
			<td class="clsFormLabelCellDefault"><?php $editfilefrm->renderLabelTableCell($value['tags']['@cfg_key'], $value['tags']['@cfg_label']); ?></td>
			<td class="clsFormFieldCellDefault">
				<select name="watermark_image_position" id="watermark_image_position" tabindex="<?php echo $tab_index;?>">
					<?php $editfilefrm->populateArray($watermark_image_position_arr, $value['value']);?>
				</select>
			</td>
		</tr>
<?php
