...
class DiscussionsFormHandler extends DiscussionHandler
	{
		...
		public function buildConditionQuery($condition='1')
			{
				...
				if($this->fields_arr['discussion_category'])
					{
		...
		/**
		 * DiscussionsFormHandler::getCategoryId()
		 *
		...
		public function getDiscussionTitles($cat_id)
			{
				$sql = 'SELECT d.discussion_title, d.discussion_id, d.user_id, u.'.$this->getUserTableField('name').' AS name, d.seo_title, d.description'.
						', d.total_solutions, d.last_post_user_id, status, publish_status, d.total_boards'.
						', DATE_FORMAT(d.last_post_date, \''.$this->CFG['mysql_format']['date_time_meridian'].'\') AS last_post_date'.
						', DATE_FORMAT(d.last_post_date, \''.$this->CFG['mysql_format']['new_date'].'\') AS last_post_date_only'.
						', DATE_FORMAT(d.last_post_date, \'%h:%m %p\') AS last_post_time_only, lp.'.$this->getUserTableField('name').' as last_post_user'.
						', lp.'.$this->getUserTableField('display_name').' as last_post_name'.
						', u.'.$this->getUserTableField('name').' as post_user, u.'.$this->getUserTableField('display_name').' as post_name'.
						' FROM '.$this->CFG['db']['tbl']['discussions'].' AS d'.
						' LEFT JOIN '.$this->CFG['db']['tbl']['users'].' AS u ON d.user_id = u.'.$this->getUserTableField('user_id').
						' LEFT JOIN '.$this->CFG['db']['tbl']['users'].' AS lp on d.last_post_user_id = lp.'.$this->getUserTableField('user_id').
						' WHERE d.pcat_id = '.$this->dbObj->Param($cat_id).' AND u.'.$this->getUserTableField('user_status').'=\'Ok\' ';
				$field_values[] = $cat_id;
				...
				if ($rs->PO_RecordCount())
					{
						while ($row = $rs->FetchRow()){
							...
							$row['myanswers']['url'] = $this->CFG['site']['url'].'admin/viewProfile.php?user_id='.$row['user_id'];
					//		$row['myanswers']['url'] = $this->CFG['site']['url'].'admin/viewMembers.php?uid='.$row['user_id'];
							$row['lastPost']['url'] = $this->CFG['site']['url'].'admin/viewProfile.php?user_id='.$row['last_post_user_id'];
						//	$row['lastPost']['url'] = $this->CFG['site']['url'].'admin/viewMembers.php?uid='.$row['last_post_user_id'];
...
$discussions->advanced_search_action = 'discussions.php?so=adv';
/*************Start navigation******/
...
$discussions->setReturnColumns(array('d.discussion_title', 'd.discussion_id', 'd.user_id', 'd.last_post_user_id', 'c.cat_name', 'u.'.$discussions->getUserTableField('name').' AS name', 'd.seo_title', 'd.description', 'd.total_boards', 'd.status', 'd.total_solutions', 'DATE_FORMAT(d.last_post_date, \''.$CFG['mysql_format']['date_time_meridian'].'\') as last_post_date', 'lp.'.$discussions->getUserTableField('name').' as last_post_user', 'lp.'.$discussions->getUserTableField('display_name').' as last_post_name', 'u.'.$discussions->getUserTableField('name').' as post_user', 'u.'.$discussions->getUserTableField('display_name').' as post_name'));
//Condition of the query
$discussions->sanitizeFormInputs($_REQUEST);
if($discussions->getFormField('cat'))
	{
		$discussions->page_title = "<a href='discussions.php'>".$LANG['discussion_search_discussion']."</a>";
...
$discussions->left_navigation_div = 'discussionsMain';
