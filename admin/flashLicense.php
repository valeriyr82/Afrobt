...
$url = explode('?',$url);
$url = str_replace('admin/flashLicense.php','',$url[0]);

$host = $err_msg = '';
$page_block_show_stat_arr = array();
$page_blocks_arr= array('message_block', 'license_form_block');
$CFG['api_url_site'] =  'http://mugshot.agriya.com';
$CFG['api_url_port'] = 80;
...
function updateMughsotVersion($mughsot_version)
	{
		global $CFG, $db;
		$sql = 'UPDATE '.$CFG['db']['tbl']['config_data'].
				' SET config_value ='.$db->Param('mughsot_version').
				' WHERE dim1 =\'mugshot\' AND dim2 =\'version\' AND config_category=\'mugshot\'';
		$stmt = $db->Prepare($sql);
		$rs = $db->Execute($stmt, array($mughsot_version));
		if (!$rs)
		    trigger_db_error($db);
...
$flashInstaler->left_navigation_div = 'generalSetting';
...
if($err_msg == '' and !isShowPageBlock('license_form_block'))
    ...
    <?php
		ECHO '<p>Mugshot license upated successfully</p>';
		...
		//Function to update mugshot version based on mugshot license
		updateMughsotVersion($version);
		setPageBlockShow('license_form_block');
	...
	<table>
    	<tr>
        	...
        	<div id="selMsgError">
             ...
             <p>Please note that the above license keys are for demo version only. Please contact agriya support to purchase the full version and provide the license key here.</p>
             </div>
             </td>
        </tr>
		<tr>
