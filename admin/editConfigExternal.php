...
if(isset($_REQUEST['config_file_name']) && $_REQUEST['config_file_name'] == 'config_encoder')
...
if(!$CFG['admin']['module']['external_login'])
										...
										switch ($variable_data_type)
											{
												case 'array':
																	...
																	if ($arr_element)
																		{
																			$arr_new_value .=  $add_key_quote .
																				...
																				stripslashes($add_key_quote) . ' => ' . stripslashes($add_value_quote) .
													...
													$var_value = ' = ' . stripslashes($arr_new_value) .';';
													break;
													...
													$comma_sep_arr_string = explode(',', stripslashes($_POST[$tags['@cfg_key']]));
													$arr_new_value = 'array(';
													if ($tags['@cfg_arr_type'] == 'key')
														{
															//Removing the tab,new line from the array element
															...
															foreach($comma_sep_arr_string as $arr_element)
																if ($arr_element)
																	$arr_new_value .= $add_key_quote . trim($arr_element) . $add_key_quote . ', ';
															$arr_new_value = stripslashes($arr_new_value);
														}
													else if ($tags['@cfg_arr_type'] == 'associative')
														{
...
$editfilefrm->left_navigation_div = 'generalLogin';
...
?>
					<tr>
						<th colspan="2"><h4><?php echo $value['tags']['@cfg_sub_head']?></h4></td>
					</tr>
<?php
						}
					switch ($variable_data_type)
...
<?php
		...
		<tr>
			<td class="clsWidthMedium clsFormLabelCellDefault"><?php $editfilefrm->renderLabelTableCell($value['tags']['@cfg_key'], $value['tags']['@cfg_label']); ?></td>
			<td class="clsFormFieldCellDefault"><?php $editfilefrm->renderTextArea($value['tags']['@cfg_key'], $no_of_row, '15', $tab_index, $new_arr); ?><?php eval($result);?></td>
		</tr>
<?php
									}
					    		break;
