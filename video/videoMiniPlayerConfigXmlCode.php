		...
		public function changeConfigBooleanValues()
			{
				...
				$this->CFG['admin']['videos']['ShowShareButton']=$this->CFG['admin']['videos']['ShowReplyButton'] = 'false';
		...
		public function getXmlCode()
			{
				...
				switch($this->fields_arr['pg'])
					{
						...
						case 'video':
							...
							if(($chkTrimedVideo AND $this->CFG['user']['user_id']!=$this->fields_arr['user_id']) Or $this->external_site)
							{
								...
								if($displayTrimmedVideo OR $this->external_site)
								{
									...
									if($this->CFG['admin']['videos']['play_list_settings'])
										{
											if($count = $this->getTotalRelatedVideo())
												{
													$configPlayListXmlcode_url = $this->CFG['site']['url'].'videoPlaylistXmlCode.php?pg=video_'.$this->fields_arr['vid'];
						...
						case 'videoactivate':
							...
							if($this->fields_arr['flv_upload_type']=='Normal')
								$video_url = $this->CFG['site']['url'].$video_folder.getHotLinkProtectionString().getVideoImageName($this->fields_arr['vid']).'.flv';
							else
								$video_url = $this->chkIsValidYoutubeUrl($this->fields_arr['video_flv_url']);
				...
				//$video_url = 'http://www.youtube.com/get_video.php?video_id=DF0jZepcQSI&t=OEgsToPDskJFtCvy5VOvUzJnw4cuGOI8&.flv';
				$this->changeConfigBooleanValues();
				if($this->fields_arr['pg'] == 'smallvideo')
				$this->CFG['admin']['videos']['SelectedSkin'] = 'mini_'.$this->CFG['admin']['videos']['SelectedSkin'];
...
<CONFIG>
	<SETTINGS>
		...
		<PLAYER_SETTINGS Name="ShowShareButton" Value="<?php echo $shareButton; ?>"/>
		<PLAYER_SETTINGS Name="ShowReplyButton" Value="<?php echo $replyButton; ?>"/>
