		...
		public function populateVideoDetails($status)
			{
				...
				$sql = 'SELECT video_server_url, user_id, video_title, flv_upload_type, video_flv_url, video_tags, video_category_id, video_tags,external_site_video_url FROM '.$this->CFG['db']['tbl']['video'].
				...
				if($row = $rs->FetchRow())
					{
						...
						//echo $this->fields_arr['photo_server_url'];
						return true;
					}
				return false;
			}
		...
		public function changeConfigBooleanValues()
			{
				...
				$this->CFG['admin']['videos']['SelectedSkin'] = $this->CFG['admin']['videos']['elite_player']['skin_name'].'.swf';
			}
		...
		public function getXmlCode()
			{
				...
				switch($this->fields_arr['pg'])
					{
						case 'video':
									...
									$isTrimmedVideo=false;
										...
										if($displayTrimmedVideo)
										{
											$trim_video_folder=$this->CFG['media']['folder'].'/'.$this->CFG['admin']['videos']['trimed_video_folder'].'/';
												...
												if($localServerMatch)
												{
													$trim_url=$trim_video_folder.getHotLinkProtectionString().getTrimVideoName($this->fields_arr['vid']).'.flv';
							...
							$sharing_url = getUrl('sharevideo','?video_id='.$this->fields_arr['vid'].'&amp;page=video',$this->fields_arr['vid'].'/'.'?page=video','','video');
							$full_screen_url = getUrl('viewvideofull','?video_id='.$this->fields_arr['vid'].'&pg=video', $this->fields_arr['vid'].'/?pg=video', '','video');
									...
									if($this->CFG['admin']['videos']['play_list_settings'])
										{
											if($count = $this->getTotalRelatedVideo())
												{
													$configPlayListXmlcode_url = getUrl('videoplaylistxmlcode','?pg=video_'.$this->fields_arr['vid'],'videoplaylistxmlcode/?pg=video_'.$this->fields_arr['vid'], '','video');
				...
				$has_list = 'yes';
				$flv_list_path_url = $this->CFG['site']['video_url'].'videolistXmlCode.php?pg=video_'.$this->fields_arr['vid'];
				$this->CFG['admin']['videos']['AutoPlay']='true';
				$this->CFG['admin']['videos']['SelectedSkin'] = $this->CFG['admin']['videos']['elite_player']['skin_name'].'.swf';
				$configPlayListXmlcode_url = '';
				$videoViewCountURL = $this->CFG['site']['video_url']."viewCount.php?videoid=".$this->fields_arr['vid'];
?>
<CONFIG>
	<SETTINGS>
		...
		<PLAYER_SETTINGS Name="FLVPath" Value="<?php echo $video_url;?>" ViewCountFile="<?php echo $videoViewCountURL ?>"/>
		<PLAYER_SETTINGS Name="FLVList" HasVideoList="<?php echo $has_list?>" FlvListPath="<?php echo $flv_list_path_url; ?>" Looping="no" RefreshList="no" />
		...
		<PLAYER_SETTINGS Name="OpenEachVideoIn" Value="self"/>
		<!--Value : Relative or Obsolute Url -->
		<PLAYER_SETTINGS Name="PlayList" Value="<?php echo $configPlayListXmlcode_url;?>" MaximumLists="1" />
		<!--Value : Relative or Obsolute Url -->
		...
		<PLAYER_SETTINGS Name="ShowMiniLogo" Value="false" Url="<?php echo $this->CFG['site']['url']; ?>images/minilogo.swf"/>
...
<?php
		...
		if(isset($_GET['next_url']) and  trim($_GET['next_url']) and isset($_GET['play_list']) and trim($_GET['play_list']) and !$this->external_site)
			{
?>
		<PLAYER_SETTINGS Name="NextUrl" Url="<?php echo $_GET['next_url'];?>" TargetWindow="_self" target="_self"  Value="<?php echo $_GET['next_url'];?>" />
		...
		public function getVideoRenderCode()
			{
				...
				$return = $this->dbObj->Insert_ID();
		 		...
		 		$stmt = $this->dbObj->Prepare($sql);
				$rs = $this->dbObj->Execute($stmt, array($fields_arr['video_id']));
