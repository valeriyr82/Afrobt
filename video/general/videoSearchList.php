 ...
 * @version $Id: VideoSearchList.php 2986 2006-12-29 05:25:10Z selvaraj_35ag05 $
 * @access public
 */

// <<<<<-------------- Class VideoUpload begins ---------------//
class VideoSearchList extends VideoHandler {
    /**
     * VideoSearchList::getPageTitle()
     * access :: Public
     *
     * @return $pg_title
     */
    ...
    public function getPageTitle()
    {
        ...
        switch ($this->fields_arr['pg']) {
            ...
            case 'videoresponseslist':
            	...
            	$title_wrapped = wordWrap_mb_ManualWithSpace($this->video_details['video_title'],$this->CFG['admin']['videos']['videolist_response_title_length'],$this->CFG['admin']['videos']['videolist_response_title_totallength']);
            ...
            default:// videonew
                ...
                if ($this->fields_arr['tags']) {
                    ...
                    $name = wordWrapManual($this->fields_arr['tags'], 20, 20, true);
                    $pg_title = str_replace('{tags_name}', $name, $pg_title);

                }
                if ($this->fields_arr['keyword']) {
                    $pg_title = $this->LANG['videolist_tagsvideo_title'];
                    $name = wordWrapManual($this->fields_arr['keyword'], 20, 20, true);
                    $pg_title = str_replace('{tags_name}', $name, $pg_title);
    ...
    /**
     * VideoSearchList::showThumbDetailsLinks()
     *
    ...
    /**
     * VideoSearchList::showVideoSearchList()
     *
     * @return
     */
    public function showVideoSearchList()
    {
        global $smartyObj;
        // for tags
        $separator = ':&nbsp;';
        ...
        while ($row = $this->fetchResultRecord()) {
            ...
            if (in_array($this->fields_arr['pg'], $need_profile_icon_arr)) {
                ...
                if (!$this->getVideoDetails(array('video_id', 'TIMEDIFF(NOW(), date_added) as date_added', 'user_id', '(rating_total/rating_count) as rating', 'total_views', 'video_server_url', 'video_ext', 'video_title', 't_width', 't_height', 'video_tags'), $row['icon_id'])) {
            ...
            if ($this->fields_arr['pg'] != 'albumlist') {
                ...
                $result_arr[$inc]['video_title_word_wrap'] = wordWrap_mb_ManualWithSpace($row['video_title'], $this->CFG['admin']['videos']['list_title_length'], $this->CFG['admin']['videos']['list_title_total_length']);
                ...
                $result_arr[$inc]['video_caption']=wordWrap_mb_ManualWithSpace($row['video_caption'],$this->CFG['admin']['videos']['caption_list_length'],$this->CFG['admin']['videos']['caption_list_total_length'],true);
            }
            if ($this->fields_arr['pg'] == 'albumlist') {
				$result_arr[$inc]['add_quicklink'] = false;
                ...
                $result_arr[$inc]['video_title'] = wordWrap_mb_ManualWithSpace($row['album_title'], $this->CFG['admin']['videos']['member_video_albums_name_length'], $this->CFG['admin']['videos']['member_video_albums_name_total_length']);
                $result_arr[$inc]['video_title_word_wrap'] = wordWrap_mb_Manual($row['album_title'], $this->CFG['admin']['videos']['list_title_length'], $this->CFG['admin']['videos']['list_title_total_length']);
    ...
    /**
     * VideoSearchList::setTableAndColumns()
     *
     * @return
     */
...
*/
    ...
    /**
     * VideoSearchList::chkValidAlbumId()
     *
     * @return
     */
    public function chkValidAlbumId()
		{
			...
			if ($row = $rs->FetchRow())
				{
					$this->ALBUM_TITLE = wordWrap_mb_ManualWithSpace($row['album_title'], $this->CFG['admin']['videos']['member_video_albums_name_length'], $this->CFG['admin']['videos']['member_video_albums_name_length'], $this->CFG['admin']['videos']['member_video_albums_name_total_length']);
    ...
    /**
     * VideoSearchList::updateVideoTagDetails()
     *
    ...
    /**
     * VideoSearchList::updateSearchCountAndResultForVideoTag()
     *
     * @param string $tag
     * @return
     */
    ...
    /**
     * VideoSearchList::updateSearchCountForVideoTag()
     *
     * @param string $tag
     * @return
     */
    ...
    public function getAlbumName()
    {
        ...
        if ($row = $rs->FetchRow())
            return wordWrap_mb_ManualWithSpace($row['album_title'], $this->CFG['admin']['videos']['member_video_albums_name_length'], $this->CFG['admin']['videos']['member_video_albums_name_length'], $this->CFG['admin']['videos']['member_video_albums_name_total_length']);
        return $this->LANG['videolist_unknown_album'];
    }

    /**
     * VideoSearchList::getCategoryName()
     *
     * @return
     */
    public function getVideoCategoryName()
    {
        ...
        if ($row = $rs->FetchRow())
            return wordWrap_mb_Manual($row['video_category_name'], $this->CFG['admin']['videos']['video_channel_title_length'], $this->CFG['admin']['videos']['video_channel_title_total_length']);
        return $this->LANG['unknown_category'];
    }

    /**
     * VideoSearchList::getMostViewedExtraQuery()
     *
     * @return
     */
    ...
    /**
     * VideoSearchList::getMostDiscussedExtraQuery()
     *
     * @return
     */
    ...
    /**
     * VideoSearchList::getMostFavoriteExtraQuery()
     *
     * @return
     */
    ...
    /**
     * VideoSearchList::getVideoCountForAlbum()
     *
...
$videos_folder = $this->CFG['media']['folder'] . '/' . $this->CFG['admin']['videos']['folder'] . '/' . $this->CFG['admin']['videos']['thumbnail_folder'] . '/';
    ...
    /**
     * VideoSearchList::populateSubCategories()
     *
     * @return
     */
    public function populateSubCategories()
    {
        ...
        while ($row = $rs->FetchRow()) {
            ...
            $row['video_category_name'] = wordWrap_mb_Manual($row['video_category_name'], $this->CFG['admin']['videos']['video_channel_title_length'], $this->CFG['admin']['videos']['video_channel_title_total_length']);
    ...
    /**
     * VideoSearchList::advancedFilters()
     *
     * @return
     */
					...
					if($i<3)
						{
							if(strlen($tags_arr[$i]) > 5 and strlen($tags_arr[$i]) > 5+3)
								$getVideoTags_arr[$i]['tags_name'] = wordWrap_mb_Manual($tags_arr[$i], 5,5,true);
							else
...
// -------------------- Code begins -------------->>>>>//
//var_dump($_POST);
$VideoSearchList = new VideoSearchList();
if (!chkAllowedModule(array('video')))
    Redirect2URL($CFG['redirect']['dsabled_module_url']);

$VideoSearchList->setDBObject($db);
$VideoSearchList->makeGlobalize($CFG, $LANG);
$VideoSearchList->setPageBlockNames(array('msg_form_error', 'msg_form_success',
									'my_videos_form', 'delete_confirm_form',
									'featured_confirm_form', 'check_all_item',
									'form_show_sub_category'));

$VideoSearchList->LANG_COUNTRY_ARR = $LANG_LIST_ARR['countries'];
$VideoSearchList->LANG_LANGUAGE_ARR = $LANG_LIST_ARR['language'];
$VideoSearchList->VIDEORUN_LENGTH_ARR = $LANG_LIST_ARR['playing_time'];
$VideoSearchList->ADDED_WITHIN_ARR = $LANG_LIST_ARR['added_within_arr'];

// default form fields and values...
$VideoSearchList->setFormField('video_id', '');
$VideoSearchList->setFormField('playlist_id', '');
$VideoSearchList->setFormField('album_id', '');
$VideoSearchList->setFormField('thumb', 'yes');
$VideoSearchList->setFormField('video_ext', '');
$VideoSearchList->setFormField('action', '');
$VideoSearchList->setFormField('act', '');
$VideoSearchList->setFormField('pg', 'videonew');
$VideoSearchList->setFormField('cid', '');
$VideoSearchList->setFormField('tags', '');
$VideoSearchList->setFormField('user_id', '0');
/**
 * ********** Page Navigation Start ********
 */
$VideoSearchList->setFormField('start', '0');
$VideoSearchList->setFormField('slno', '1');
$VideoSearchList->setFormField('sid', '');
$VideoSearchList->setFormField('video_ids', array());
$CFG['video_tbl']['numpg'] = 28;
$VideoSearchList->setFormField('numpg', $CFG['video_tbl']['numpg']);
$condition = '';
$VideoSearchList->setFormField('advanceFromSubmission', '');
$VideoSearchList->setFormField('video_country', '');
$VideoSearchList->setFormField('video_language', '');
$VideoSearchList->setFormField('run_length', '');
$VideoSearchList->setFormField('added_within', '');
$VideoSearchList->setFormField('keyword', '');
$VideoSearchList->setFormField('video_owner', '');
$VideoSearchList->setFormField('searchkey', '');
$VideoSearchList->setFormField('default', 'Yes');
$VideoSearchList->setFormField('myvideo', 'No');
$VideoSearchList->setFormField('myfavoritevideo', 'No');

$VideoSearchList->recordsFound = false;

$VideoSearchList->setMinRecordSelectLimit($CFG['data_tbl']['min_record_select_limit']);
$VideoSearchList->setMaxRecordSelectLimit($CFG['data_tbl']['max_record_select_limit']);
$VideoSearchList->setNumPerPageListArr($CFG['data_tbl']['numperpage_list_arr']);
$VideoSearchList->setTableNames(array());
$VideoSearchList->setReturnColumns(array());

$VideoSearchList->default_url = getUrl('videolist','?pg=videonew','videonew/','','video');
/**
 * *********** page Navigation stop ************
 */
$VideoSearchList->setAllPageBlocksHide();
$VideoSearchList->setPageBlockShow('my_videos_form');

$VideoSearchList->sanitizeFormInputs($_REQUEST);

if($VideoSearchList->getFormField('default') == 'No')
	{
		$VideoSearchList->setFormField('pg', (isset($_GET['pg']) && !empty($_GET['pg']))?$_GET['pg']:$_REQUEST['pg']);
	}

if($VideoSearchList->getFormField('pg')== 'myvideo')
	$VideoSearchList->setFormField('myvideo', 'Yes');

if($VideoSearchList->getFormField('pg')== 'myfavoritevideos')
	$VideoSearchList->setFormField('myfavoritevideo', 'Yes');

$videoActionNavigation_arr['video_list_url_0'] = getUrl('videolist', '?pg='.$VideoSearchList->getFormField('pg').'&action=0', $VideoSearchList->getFormField('pg').'/?action=0', '', 'video');
$videoActionNavigation_arr['video_list_url_1'] = getUrl('videolist', '?pg='.$VideoSearchList->getFormField('pg').'&action=1', $VideoSearchList->getFormField('pg').'/?action=1', '', 'video');
$videoActionNavigation_arr['video_list_url_2'] = getUrl('videolist', '?pg='.$VideoSearchList->getFormField('pg').'&action=2', $VideoSearchList->getFormField('pg').'/?action=2', '', 'video');
$videoActionNavigation_arr['video_list_url_3'] = getUrl('videolist', '?pg='.$VideoSearchList->getFormField('pg').'&action=3', $VideoSearchList->getFormField('pg').'/?action=3', '', 'video');
$videoActionNavigation_arr['video_list_url_4'] = getUrl('videolist', '?pg='.$VideoSearchList->getFormField('pg').'&action=4', $VideoSearchList->getFormField('pg').'/?action=4', '', 'video');
$videoActionNavigation_arr['video_list_url_5'] = getUrl('videolist', '?pg='.$VideoSearchList->getFormField('pg').'&action=5', $VideoSearchList->getFormField('pg').'/?action=5', '', 'video');
...
if($VideoSearchList->getFormField('pg') == 'videomostviewed'
	OR $VideoSearchList->getFormField('pg') == 'videomostdiscussed'
		OR $VideoSearchList->getFormField('pg') == 'videomostfavorite')
	{
		if(!$VideoSearchList->getFormField('action')) $VideoSearchList->setFormField('action', '0');
		$sub_page = 'videoMostViewed_'.$VideoSearchList->getFormField('action');
...
if($VideoSearchList->getFormField('searchkey'))
	{
		$VideoSearchList->setFormField('keyword', $VideoSearchList->getFormField('searchkey'));
		$VideoSearchList->setFormField('avd_search', '1');
		$VideoSearchList->getFormField('keyword');
	}

$start = $VideoSearchList->getFormField('start');
if ($VideoSearchList->getFormField('cid') && !$VideoSearchList->getFormField('sid') && $CFG['admin']['videos']['sub_category'])
{
    $VideoSearchList->setPageBlockShow('form_show_sub_category');
}
if ($VideoSearchList->getFormField('pg') == 'myalbumvideolist')
{
    if (!$VideoSearchList->chkValidAlbumId())
	{
        $VideoSearchList->setAllPageBlocksHide();
        $VideoSearchList->setPageBlockShow('videolist_msg_form_error');
        $VideoSearchList->setCommonErrorMsg($LANG['videolist_msg_error_sorry'] . ' ' . $LANG['videolist_invalid_album']);
    }
}
...
if ($VideoSearchList->isFormPOSTed($_POST, 'avd_reset'))
	{
		$VideoSearchList->setFormField('keyword', '');
		$VideoSearchList->setFormField('video_owner', '');
		$VideoSearchList->setFormField('video_country', '');
...
if ($VideoSearchList->isFormPOSTed($_POST, 'yes'))
{

    if ($VideoSearchList->getFormField('act') == 'delete')
	{
        $videos_arr = explode(',', $VideoSearchList->getFormField('video_id'));
        $VideoSearchList->deleteVideos($videos_arr, $CFG['user']['user_id']);
    }
    if ($VideoSearchList->getFormField('act') == 'set_avatar')
	{
        $VideoSearchList->setFeatureThisImage($VideoSearchList->getFormField('video_id'), $CFG['user']['user_id']);
        $VideoSearchList->setPageBlockShow('msg_form_error');
        $VideoSearchList->setCommonErrorMsg($LANG['videolist_msg_success_set_avatar']);
    }
    if ($VideoSearchList->getFormField('act') == 'favorite_delete')
	{
        $video_id_arr = explode(',', $VideoSearchList->getFormField('video_id'));
        foreach($video_id_arr as $video_id)
		{
               $VideoSearchList->deleteFavoriteVideo($video_id, $CFG['user']['user_id']);
        }
    }
    if ($VideoSearchList->getFormField('act') == 'playlist_delete')
	{
        $video_id_arr = explode(',', $VideoSearchList->getFormField('video_id'));
        foreach($video_id_arr as $video_id)
		{
            $VideoSearchList->deletePlaylistVideo($video_id);
        }
    }
    if ($VideoSearchList->getFormField('act') == 'set_playlist_thumb')
	{
        $video_id = $VideoSearchList->getFormField('video_id');
        $VideoSearchList->setPlaylistThumbnail($video_id);
    }
    if ($VideoSearchList->getFormField('act') == 'set_album_thumb')
	{
        $video_id = $VideoSearchList->getFormField('video_id');
        $album_id = $VideoSearchList->getFormField('album_id');
        $VideoSearchList->updateAlbumProfileImage($video_id, $album_id);
    }
}

$VideoSearchList->category_name = '';

if ($VideoSearchList->isShowPageBlock('form_show_sub_category'))
{
    $VideoSearchList->populateSubCategories();
    $VideoSearchList->category_name = $VideoSearchList->getVideoCategoryName();
    $VideoSearchList->LANG['videolist_category_title'] = str_replace('{category}', $VideoSearchList->category_name, $LANG['videolist_category_title']);
}
$VideoSearchList->LANG['videolist_title'] = $VideoSearchList->getPageTitle();
...
$VideoSearchList->showThumbDetailsLinks_arr = $VideoSearchList->showThumbDetailsLinks($thum_details_arr);
if ($VideoSearchList->isShowPageBlock('msg_form_error'))
{
    $VideoSearchList->msg_form_error['common_error_msg'] = $VideoSearchList->getCommonErrorMsg();
}
if ($VideoSearchList->isShowPageBlock('msg_form_success'))
{
    $VideoSearchList->msg_form_success['common_error_msg'] = $VideoSearchList->getCommonErrorMsg();
}
if ($VideoSearchList->isShowPageBlock('my_videos_form'))
{
    /**
     * ***** navigtion continue********
     */
    $VideoSearchList->setTableAndColumns();
    $VideoSearchList->buildSelectQuery();
    // $VideoSearchList->buildConditionQuery($condition);
    $VideoSearchList->buildQuery();
    //$VideoSearchList->printQuery();
    ...
    if (in_array($VideoSearchList->getFormField('pg'), $group_query_arr))
        $VideoSearchList->homeExecuteQuery();
    else
        $VideoSearchList->executeQuery();
    /**
     * ****** Navigation End *******
     */
    $VideoSearchList->my_videos_form['anchor'] = 'anchor';
    $VideoSearchList->isResultsFound = $VideoSearchList->isResultsFound();
    ...
    if ($VideoSearchList->isResultsFound())
	{
		//$smartyObj->assign('smarty_paging_list', $VideoSearchList->populatePageLinksGET($VideoSearchList->getFormField('start'), $paging_arr));
		$smartyObj->assign('smarty_paging_list', $VideoSearchList->populatePageLinksPOST($VideoSearchList->getFormField('start'), 'seachAdvancedFilter'));
        switch ($VideoSearchList->getFormField('pg'))
		{
            case 'useralbumlist':
                if (!$VideoSearchList->isMe($VideoSearchList->ALBUM_USER_ID))
                    break;
            case 'myvideos':
                $VideoSearchList->setPageBlockShow('check_all_item');
                break;
            case 'myfavoritevideos':
                $VideoSearchList->setPageBlockShow('check_all_item');
                break;
        ...
        $VideoSearchList->my_videos_form['showVideoList'] = $VideoSearchList->showVideoSearchList();
    }
}
// include the header file
$VideoSearchList->includeHeader();
?>
...
$smartyObj->display('videoSearchList.tpl');
