...
class googleVideoSitemap extends FormHandler
{
	...
	{
		$sql='SELECT video_id,user_id,video_ext,TIME_FORMAT(playing_time,\'%H:%i:%s\') as playing_time,video_title,video_caption,video_tags,allow_embed,v.date_added,video_server_url,t_width,t_height,total_views,video_category_name,video_category_type,(rating_total/rating_count) as video_rating FROM '.
		$this->CFG['db']['tbl']['video'].' as v JOIN '.$this->CFG['db']['tbl']['video_category'].' as vc ON vc.video_category_id = v.video_category_id
		WHERE video_status=\'Ok\' AND flagged_status!=\'Yes\' AND 	video_access_type=\'Public\'
		...
		{
			$viewVideoPageUrl=getUrl('viewvideo','?video_id='.$row['video_id'].'&amp;video_title='.$this->changeTitle($row['video_title']), $row['video_id'].'/'.$this->changeTitle($row['video_title']).'/','','video');
			if($this->CFG['admin']['videos']['full_length_video']!='All')
			{
			$contentLocation=$row['video_server_url'].$video_folder.getVideoName($row['video_id']).'.flv';
			}
			else
			{
				$video_folder = $this->CFG['media']['folder'].'/'.$this->CFG['admin']['videos']['trimed_video_folder'].'/';
				$contentLocation =$row['video_server_url'].$video_folder.getTrimVideoName($row['video_id']).'.flv';
			}
			/*if($this->CFG['admin']['videos']['SelectedPlayer']=='elite')
			{
				$flv_player_url= $this->CFG['site']['url'].$this->CFG['media']['folder'].'flash/flvPlayer/flvplayer_elite.swf';
			}
			else
			{*/
				$flv_player_url = $this->CFG['site']['video_url'].'embedPlayer.php?vid='.mvFileRayzz($row['video_id']).'_'.$row['video_id'];
			//}
			$arguments_play = 'pg=video_'.$row['video_id'].'_no_'.getRefererForAffiliate();

			/*if($this->CFG['admin']['videos']['SelectedPlayer']=='elite')
			{
				$flv_player_url_embed= $this->CFG['site']['url'].$this->CFG['media']['folder'].'flash/flvPlayer/flvplayer_elite.swf';
			}
			else
			{*/
				$flv_player_url_embed= $this->CFG['site']['video_url'].'embedPlayer.php?vid='.mvFileRayzz($row['video_id']).'_'.$row['video_id'];
			//}
			if($this->CFG['admin']['videos']['SelectedPlayer']=='elite')
			{
			...
			$playerUrl=$flv_player_url.'&amp;embedded_config='.$configXmlcode_url.$arguments_play;
			...
			?><url>
 			<loc><?php echo $viewVideoPageUrl;?></loc>
    			...
    			<video:content_loc><?php echo $contentLocation;?></video:content_loc>
      			<video:player_loc allow_embed="<?echo $row['allow_embed'];?>"><?echo $playerUrl;?></video:player_loc>
      			...
      			{?>
				<video:tag><![CDATA[<?php echo $tag;?>]]></video:tag>
				<?}?>
				<video:category><![CDATA[<?php echo $row['video_category_name'];?>]]></video:category>
