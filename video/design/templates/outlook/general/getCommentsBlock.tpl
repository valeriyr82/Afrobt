<script type="text/javascript"	src="{$CFG.site.video_url}js/light_comment.js"></script>
<div class="clsOverflow">
    <form name="addComments" id="addComments" method="post" action="{$myobj->getCurrentUrl()}" autocomplete="off" onsubmit="return false">
        ...
        {if $CFG.admin.videos.captcha AND $CFG.admin.videos.captcha_method =='honeypot'}
            {$myobj->hpSolutionsRayzz()}
        {/if}
            <table>
                <tr>
                    <td>
                        <textarea class="clsEmbedTextFields" name="comment" id="comment" tabindex="{smartyTabIndex}" rows="5" cols="60"></textarea>
                    </td>
                </tr>
        {if $CFG.admin.videos.captcha}
            {if $CFG.admin.videos.captcha_method =='recaptcha' and $CFG.captcha.public_key and $CFG.captcha.private_key}
              <tr>
            ...
            {elseif $CFG.admin.videos.captcha_method =='image'}
                    <tr>
                        <td>
        ...
        {/if}
                <tr>
                    <td>
                    {if $CFG.admin.videos.captcha}
                        {if $CFG.admin.videos.captcha_method =='recaptcha'}
                            <div class="clsGreyButtonLeft"><div class="clsGreyButtonRight"><input type="button" onClick="return light_addToCommentRecaptcha('{$CFG.site.url}video/viewVideo.php?ajax_page=true&amp;video_id={$myobj->getFormField('video_id')}&amp;vpkey={$myobj->getFormField('vpkey')}&amp;show={$myobj->getFormField('show')}');" name="post_comment" id="post_comment" tabindex="{smartyTabIndex}" value="{$LANG.viewvideo_submit}" /></div></div>
                        {elseif $CFG.admin.videos.captcha_method =='image'}
                            <div class="clsGreyButtonLeft"><div class="clsGreyButtonRight"><input type="button" onClick="return light_addToComment('{$CFG.site.url}video/viewVideo.php?ajax_page=true&amp;video_id={$myobj->getFormField('video_id')}&amp;vpkey={$myobj->getFormField('vpkey')}&amp;show={$myobj->getFormField('show')}');" name="post_comment" id="post_comment" tabindex="{smartyTabIndex}" value="{$LANG.viewvideo_submit}" /></div></div>
                        {elseif $CFG.admin.videos.captcha_method =='honeypot'}
                            <div class="clsGreyButtonLeft"><div class="clsGreyButtonRight"><input type="button" onClick="return light_addToCommentHoneyPot('{$CFG.site.url}video/viewVideo.php?ajax_page=true&amp;video_id={$myobj->getFormField('video_id')}&amp;vpkey={$myobj->getFormField('vpkey')}&amp;show={$myobj->getFormField('show')}', '{$myobj->phFormulaRayzz()}')" name="post_comment" id="post_comment" tabindex="{smartyTabIndex}" value="{$LANG.viewvideo_submit}" /></div></div>
                        {/if}
                        {else}
