...
<div id="selArticleWriting" class="clsPageHeading">
	<div class="clsOverflow sectionHeading"><div class="clsSectionHeadingLeft"><h2>{if $myobj->chkIsEditMode()} {$LANG.articlewriting_edit_title} {else} {$LANG.articlewriting_title} {/if}</h2></div>
	{if $myobj->chkIsEditMode()}<div class="clsSectionHeadingRight"><h4><a href="{$myobj->article_writing_form.manage_attachments_url}">{$LANG.articlewriting_manage_attachments}</a></h4></div>{/if}</div>
  	{*{if $CFG.admin.articles.auto_activate || $CFG.admin.is_logged_in}{if $myobj->chkIsEditMode()}<div class="clsSectionHeadingRight"><h2><a href="{$myobj->article_writing_form.manage_attachments_url}">{$LANG.articlewriting_manage_attachments}</a></h2></div>{/if}{/if}</div>*}
